...
- Desktop source commit：`53dc4fd88681ab0a4bd839f57d657b43573827dd`
...
- 精确候选包已验证 Relay 浏览器闭环：以包内 `NDMRelay` 启动隔离的 Chrome for Testing 151，真实 MV3 Service Worker、弹窗、内容脚本和 WebSocket Bridge 均已运行。弹窗显示已连接与 `v1.4.0`；模拟 Host 离线后显示“当前下载会留在浏览器”的明确说明，恢复连接成功；“显示 NDM”触发真实 `focusApp` 广播；页面识别到 1 个 MP4 候选，点击后任务出现在精确候选 Electron 中，并由 Swift Host 完成 3,209,032 字节下载。测试任务、下载和隔离浏览器资料均已移入废纸篓，设置恢复原值，浏览器与 Electron 控制台无错误；`qa:relay-browser` 保留供复验。
...
2. **尚未在用户日常使用的可见 Google Chrome 窗口完成一次人工视觉复核。** 隔离 Chrome for Testing 已证明真实扩展运行、弹窗交互、媒体识别、任务交接和错误恢复；但用户 Chrome 控制桥仍要求另开窗口授权，因此工具栏固定、实际浏览器资料下的视觉状态仍待人工确认。
...
1. 获得用户许可后，在日常使用的可见 Google Chrome 中加载包内 Relay，人工复核工具栏、弹窗与站点内控件；无需重搭自动化环境。
2. 在网络条件允许时重新验证真实 YouTube 合集。
3. 配置 Developer ID 后重新签名、公证并再次执行本页全部包级检查。
