...
核查日期：2026-09-14；解锁后续验收：2026-09-15。用户要求覆盖日常下载可靠性、任务管理/自动化、新协议三个方向。这个范围用于分批交付，不把研究、代码完成、测试通过和安装可用混为一谈。
...
- 2026-09-15 解锁后，已安装版本为 `2026091502`，包含功能检查点 `97c5495` 及后续 Range/大任务库修复 `c394853`，正式签名和辅助工具校验通过。旧 `dist` 的 `2026091404` 不应覆盖新版。当前继续真实界面验收，并修复新文件选择器未绑定请求窗口的问题；Goal仍未完成。
...
### 2026-09-15 解锁后的安装核对与原生交互

- 当前安装包 `2026091502` 的 ASAR 与 Host 哈希逐字匹配 [Range 修复安装记录](../qa/http-range-fallback-2026-09-15.md)。该包从 `97c5495` 加后续修复构建，包含本功能批次；未提交 Douyin 工作仍保留在主工作区，没有混入这次安装。无需为取得本功能批次重新安装旧 `2026091404`。
- 今日重新建立只含编号与状态的私有基线：3728条，326完成、417错误、6暂停、2979不完整，0活动任务。排序编号SHA256 `c740f0ee8a2877198255bdb81b10a8f99bc3edacda0350bfa340aec10c3c824d`。相较昨日新增记录属于两次验收之间的正常使用，不以昨日3712条覆盖今日基线。
- 原生界面已确认安装版本、设置备份、下载完成后动作、等待队列、周期限速、目录规则及协议入口可见。完整创建/文件交付尚待下述验收，不能将入口可见记作端到端通过。
- 真实已安装二进制使用独立 `user-data-dir`、`NDM_SUPPORT_DIR`、Host64426/Bridge64427；无生产任务库副本。私有双文件种子和三任务镜像清单仅使用回环HTTP50954，禁用辅助引擎发现和非回环监听。尚未提交创建任务；服务仅有各32字节的两次标记预检。
- 原生UI工具在点击和窗口切换时有延迟反馈；关闭独立文件选择器后多次返回 `timeoutReached`。应用主线程采样为空闲事件循环，Host可正常返回空任务库，不能断言为应用死锁。源码另确认五个新选择器未传父窗口，独立补丁将其绑定到发起请求的窗口，并覆盖异步队列归属、取消重选和窗口销毁。
- 只停止自有空任务库QA进程，随后恢复正式应用；原生界面重新显示3728条任务，RPC核对全部编号与今日基线一致，0活动任务。待窗口绑定补丁验证包完成后继续隔离原生验收。

### 2026-09-16 原生界面验收与新增媒体问题

- `f3bd7f3` 将五个原生文件/目录选择器绑定至请求窗口，保留跨异步队列的窗口归属；647项TS测试中639通过、8项显式集成跳过，类型检查与构建通过。签名验证包为`2026091503`，尚未替换正式`2026091502`。
- 真实Electron全屏窗口已通过：本地种子选择、取消后重选、两文件元数据预览、选择前零文件载荷、只选一份768KiB文件、做种期间保持非完成、界面停止做种后发布、界面SHA-256显示校验一致。另一文件无实际载荷、没有发布。
- SFTP界面使用本机隔离服务器、合成账号与固定主机公钥完成256KiB文件，SHA-256一致。三任务aria2清单预览时没有HTTP请求；界面创建三项、重排C至B之前，实际请求顺序A→C→B；三份512KiB成品均校验一致，首镜像404均正确切换。
- 设置备份已实际导出JSON，界面将单项设置从关闭改为开启后，再导入原备份；预览正确显示一项变更，应用后显示已核对保存结果。目录规则的原生目录选择、主机+扩展名组合预览和保存通过；周期规则128KiB/s保存通过，当前位于规则时段外。时段内的实际限速效果沿用前述独立Host实测，未把界面保存当作再次测速。
- 隔离验收回执`/tmp/ndm-native-gui-acceptance-20260916.json`记录五个成品摘要和实际请求顺序。测试NDM经应用菜单正常退出，HTTP/SFTP夹具已停止；原有任务库没有被夹具修改。原生UI工具偶有旧状态/焦点错误，重新绑定后可操作；用户亦确认“添加下载”正常打开。
- 用户新增抖音会话和YouTube进度问题，安装因此延后至同批修复后统一执行。新的浏览器来源与媒体完整性交付证据见[媒体会话与成品验收](../qa/media-session-and-delivery-2026-09-16.md)。Windows操作系统实机和真实关机/睡眠动作仍没有执行。

### 2026-09-16 统一安装与媒体验收进度

- 最终功能及媒体源码集成至`ef47d15`，签名包2026091601已替换`/Applications/NDM.app`；设置页已核对版本及Native Swift engine。应用正常退出后保留旧包与私有数据库备份，安装前后3729条下载、708条请求头、33条Relay回执及其他原表逐字段摘要一致。
- TS655通过/8跳过、Relay243通过、原生Engine673项中28跳过及Core555、Bridge32、Swift Testing 11全部零失败。新Release Host与实际签名Host均通过页面查询和旧扩展捕获入口的真实隔离传输；下载文件、来源、显式名称、创建回执及清理均核对。具体哈希和回执见媒体验收记录。
- 原YouTube链接的指定1080p高码率实测成品44,041,630字节，1920×1080、完整音轨、约162秒，全文件解码通过；估算峰值收敛及缺片失败已修复。原用户文件和其200MB估算峰值未直接复现，不据此声称原文件已鉴定。
- Chrome当前主资料已确认是Default（用户1），但真实安装版仍误报未找到资料，正在诊断应用读取失败。新扩展源码1.4.16等待用户手动重新加载；浏览器工具禁止操作扩展管理页，未改用其他通道绕过。原抖音页面的新版真实交接及成品仍待验收，独立API403未记为已解决。
- `24133e7`补齐资料读取的短读处理、有限重试及折叠错误诊断，未将尚未知的GUI根因推断为权限或Profile问题。673项TS通过、9项显式集成跳过，类型检查和构建通过；`b688d2f`的1602签名包及安装暂存校验完成。正常退出前Mac重新锁屏，等待用户解锁；正式应用仍运行1601，1602未安装。解锁后需重新核对任务基线及实际设置页，不把候选包生成视为资料读取问题已解决。
