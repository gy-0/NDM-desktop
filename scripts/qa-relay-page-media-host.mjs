...
const root = await mkdtemp(join(tmpdir(), 'ndm-page-media-host-'))
...
try {
  ...
  assert.equal(replay.task.id, created.task.id); assert.equal((await request('list')).tasks.length, 1)
  ...
  console.log(JSON.stringify({ passed: true, isolatedSources: 2, discoveryTransfers: 0, selectedProfileOnly: true, actualBytes: payload.length,
    sha256: createHash('sha256').update(payload).digest('hex'), explicitFilenamePreserved, headersAndPagePreserved: true, concurrentAndDisconnectedReplaySameTask: true, tasks: 1 }))
} finally {
  for (const socket of workers) socket.close()
