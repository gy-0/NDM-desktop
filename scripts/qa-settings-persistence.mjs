...
try {
  ...
  await settings.locator('summary').getByText('高级连接设置', { exact: true }).click()
  await settings.getByRole('group', { name: '单任务最大连接数', exact: true }).getByRole('button', { name: '8', exact: true }).click()
