...
try {
  ...
  assert.equal(restored.completedBytes, durablePrefixBytes, 'Restarted paused task displays its committed progress')
  assert.equal(restored.fileSize, payloads['/a.bin'].length)
  assert.ok(!transfers.some(row => row.generation === 2), 'Reading restored progress must not issue network requests')
