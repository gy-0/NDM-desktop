...
| `tasks` | `sample`, `empty`, `showcase` (twelve synthetic tasks and authored preview illustrations) |
...
For the compact download cards and transfer island, open `/scripts/fixtures/renderer-preview/app.html?view=main&theme=dawn&tasks=showcase&layout=cards`. The illustrations are fixture assets; production previews still come from each actual file. Use `node scripts/qa-component-motion.mjs` after a production build for the separate Electron integration check. Set `NDM_QA_APP_PATH` to a packaged macOS executable to check that artifact with the same isolated fixture.
