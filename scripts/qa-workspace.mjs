  ...
  if (!process.argv.includes('--capture-only')) {
    ...
    await check('chosen destination survives late missing defaults and failed confirmation keeps focus', async () => {
      ...
      await page.evaluate(() => {
        ...
        window.__qa.update(102, { awaitingDestination: true, folderPath: '', errorText: '#diag:unsupportedDestination' })
      })
      const dialog = page.getByRole('dialog', { name: '选择保存目录' })
      await dialog.getByText('当前保存位置暂不支持安全保存文件。请选择 Mac 本机目录继续，任务会保留。', { exact: true }).waitFor()
      ...
      assert.equal(await dialog.getByText('当前保存位置暂不支持安全保存文件。请选择 Mac 本机目录继续，任务会保留。', { exact: true }).count(), 0)
      ...
      await confirm.click()
      await page.evaluate(() => window.__qa.finishDestination({ ok: false, errorKind: 'unsupportedDestination' }))
      await dialog.getByText('当前保存位置暂不支持安全保存文件，请选择 Mac 本机目录。', { exact: true }).waitFor()
      await confirm.click()
    ...
    await check('Inspector file and source actions retain feedback without changing another task', async () => {
      await reset()
      await page.evaluate(() => {
        window.ndm.revealFile = () => new Promise(resolve => { window.__qa.finishReveal = resolve })
        window.ndm.openExternal = async () => false
        window.__qa.update(102, { pageURL: 'https://example.com/source-page' })
      })
      await row(102).click()
      const pane = page.locator('#task-inspector')
      const reveal = pane.getByRole('button', { name: /中显示保存位置/ })
      await reveal.click()
      assert.equal(await reveal.isEnabled(), false)
      await page.evaluate(() => window.__qa.finishReveal('parent-opened'))
      await pane.getByText('文件已不在原位置，已打开原保存文件夹', { exact: true }).waitFor()
      await page.waitForFunction(() => document.activeElement?.getAttribute('aria-label')?.includes('中显示保存位置'))
      await pane.getByRole('button', { name: '在浏览器中打开来源网页', exact: true }).click()
      await pane.locator('[data-detail-field="来源网页"]').getByText('未能打开浏览器，请重试。', { exact: true }).waitFor()
      await pane.getByRole('button', { name: '在浏览器中打开下载链接', exact: true }).click()
      await pane.locator('[data-detail-field="下载链接"]').getByText('未能打开浏览器，请重试。', { exact: true }).waitFor()
      await reveal.click()
      await row(103).click()
      await page.evaluate(() => window.__qa.finishReveal('parent-opened'))
      assert.equal(await pane.getByText('文件已不在原位置，已打开原保存文件夹', { exact: true }).count(), 0)
      assert.equal(await pane.getByText('未能打开浏览器，请重试。', { exact: true }).count(), 0)
      await reset()
    })
    ...
    await check('reduced motion stops the pending settings spinner without losing save state', async () => {
      ...
      await page.evaluate(() => {
        const original = window.ndm.request
        window.ndm.request = (op, extra) => op === 'updateSettings'
          ? new Promise(resolve => { window.__qa.finishMotionSave = async () => {
            const reply = await original('getSettings')
            resolve({ ok: true, settings: { ...reply.settings, ...extra } })
          } })
          : original(op, extra)
      })
      await page.getByRole('button', { name: '设置', exact: true }).click()
      await page.getByRole('navigation', { name: '设置分类' }).getByRole('button', { name: '下载', exact: true }).click()
      const toggle = page.getByRole('switch', { name: '浏览器下载前选择保存目录', exact: true })
      await toggle.click()
      assert.equal(await toggle.isEnabled(), false)
      assert.equal(await toggle.getAttribute('aria-busy'), 'true')
      const iterations = () => toggle.locator('.t-toggle-thumb').evaluate(el => getComputedStyle(el, '::after').animationIterationCount)
      assert.equal(await iterations(), '1')
      await page.emulateMedia({ reducedMotion: 'no-preference' })
      assert.equal(await iterations(), 'infinite')
      await page.emulateMedia({ reducedMotion: 'reduce' })
      assert.equal(await iterations(), '1')
      await page.evaluate(() => window.__qa.finishMotionSave())
      await page.waitForFunction(() => document.querySelector('[role="switch"][aria-label="浏览器下载前选择保存目录"]')?.getAttribute('aria-busy') !== 'true')
    ...
    await check('unread settings cannot be saved and recovery keeps save failures visible', async () => {
      ...
      assert.equal(await panel.getByRole('button', { name: '保存并关闭规则' }).evaluate(e => e === document.activeElement), true)
      ...
      await panel.getByRole('button', { name: '放弃编辑并重新读取' }).press('Enter')
      await panel.getByText('已重新读取周期限速规则。', { exact: true }).waitFor()
      assert.equal(await panel.locator('[aria-label="周期限速操作结果"]').evaluate(e => e === document.activeElement), true)
      await reset()
    })
    ...
    await check('single creation reconciles lost replies and retains unknown receipts across close', async () => {
      ...
      await page.waitForFunction(() => document.activeElement?.textContent?.includes('确认 1 项'))
      ...
      await page.waitForFunction(() => document.activeElement?.getAttribute('aria-label') === '下载链接')
      assert.equal(await page.evaluate(() => window.__qa.single.adds), 2)
