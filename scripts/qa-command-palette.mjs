...
try {
  ...
  await capture('04-copy-link-search')
  await input.fill('预览')
  assert.equal(await palette.getByRole('option').count(), 1)
  ...
  await win.keyboard.press('Escape')
  await settings.waitFor({ state: 'hidden' })
  assert.equal(await row(2).getAttribute('aria-pressed'), 'true', 'Escape from settings must not clear the selected task underneath')
