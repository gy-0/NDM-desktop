...
try {
  ...
  for(const name of ['通用','外观与声音','下载','网络','浏览器扩展']) assert.equal(await settings.getByRole('button',{name,exact:true}).evaluate(el=>getComputedStyle(el).fontSize),'14px')
  assert.equal(await settings.getByRole('button',{name:'返回应用',exact:true}).evaluate(el=>getComputedStyle(el).fontSize),'14px')
