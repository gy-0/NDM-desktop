...
export function BrowserPageMediaPicker({ pageURL, disabled, choice, onSelect, autoRead = false }: {
  ...
  return <section className="mt-3 rounded-surface border border-line/70 bg-ink/20 p-3" aria-label="浏览器页面视频">
    <div className="flex flex-wrap items-center gap-2">
      <button type="button" disabled={disabled || busy} onClick={() => void read()} className="h-8 rounded-control bg-copper px-3 text-meta font-medium text-on-accent disabled:opacity-50">从浏览器页面读取</button>
      <button type="button" disabled={disabled || browserAction.busy} aria-busy={browserAction.busy || undefined} onClick={() => void browserAction.open()} className="h-8 rounded-control border border-line px-3 text-meta text-fog disabled:opacity-50">{browserAction.busy ? '正在打开…' : '在浏览器中打开'}</button>
    </div>
    <p className="mt-2 text-caption leading-relaxed text-mist">请在原浏览器的当前标签页打开并播放此视频。只读取该页面已捕获的版本，再由你选择下载。</p>
    {browserAction.error ? <p role="status" className="mt-2 text-meta text-clay">{browserAction.error}</p> : null}
    {busy ? <div className="mt-2"><LoadingMark label="正在读取浏览器页面…" /></div> : null}
    {error ? <p role="status" className="mt-2 text-meta text-clay">{error}</p> : null}
    {sources.map((source, index) => <fieldset key={source.sourceToken} disabled={disabled || busy} className="mt-3 min-w-0 border-t border-line/70 pt-2">
      <legend className="max-w-full truncate pr-2 text-meta text-paper">{browserLabel(source.browser as SessionBrowserID) || source.browser} · 页面 {index + 1}{source.incognito ? ' · 隐身窗口' : ''} · {source.title || '视频页面'}</legend>
      <div className="mt-1 grid gap-1.5">
        {source.items.map(item => {
          ...
          return <button key={item.mediaKey} type="button" aria-pressed={selected}
            ...
            className={`flex items-center justify-between gap-3 rounded-control border px-3 py-2 text-left text-meta transition-colors disabled:opacity-50 ${selected ? 'border-copper bg-copper/10 text-paper' : 'border-line text-fog hover:bg-line/40'}`}>
