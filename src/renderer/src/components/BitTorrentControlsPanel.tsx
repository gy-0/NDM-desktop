...
const button = 'inline-flex items-center justify-center gap-1.5 rounded-control border border-line px-2.5 py-2 text-[12px] text-paper hover:bg-raised disabled:opacity-40'
const input = 'w-full rounded-control border border-line bg-raised px-2.5 py-2 text-[12px] text-paper outline-none focus:border-copper/50 disabled:opacity-50'
...
export function BitTorrentControlsPanel({ taskID, onChanged }: { taskID: number; onChanged?: () => void | Promise<void> }) {
  ...
  return <section aria-label="BT 连接与分享" className="space-y-3 border-t border-line pt-4">
    <div className="flex items-center justify-between gap-2"><h3 className="text-[13px] font-medium text-paper">BT 连接与分享</h3><button type="button" className={button} disabled={busy || loading} onClick={() => void run(async () => refresh())}><RefreshCw size={13} />刷新</button></div>
    {loading ? <p role="status" className="text-[12px] text-mist">正在读取 BT 配置…</p> : null}
    {state && draft ? <>
      {!canConfigureBT(state.phase) ? <div className="space-y-2"><p className="text-[12px] text-mist">配置与手动 peer 仅在任务暂停后修改。当前连接信息会自动刷新。</p>{['downloading', 'seeding', 'checking', 'metadata'].includes(state.phase) ? <button type="button" className={button} disabled={busy} onClick={() => void run(async current => {
      ...
      <details className="rounded-control border border-line p-3" open>
        <summary className="cursor-pointer text-[12px] font-medium text-paper">Tracker 与 WebSeed</summary>
        <div className="mt-3 space-y-3">
          <label className="block space-y-1 text-[12px] text-mist"><span>Tracker · 每行“层级 地址”，层级为 0–255；省略则为 0</span><textarea aria-label="BT Tracker 列表" className={`${input} min-h-[100px] font-mono`} value={draft.trackers} disabled={!editable} spellCheck={false} onChange={event => edit({ trackers: event.target.value })} /></label>
          <label className="block space-y-1 text-[12px] text-mist"><span>WebSeed · 每行一个 HTTP 或 HTTPS 地址</span><textarea aria-label="BT WebSeed 列表" className={`${input} min-h-[80px] font-mono`} value={draft.webSeeds} disabled={!editable} spellCheck={false} onChange={event => edit({ webSeeds: event.target.value })} /></label>
          <p className="text-[11px] text-mist">保存会替换本任务的列表；空列表表示清除。带令牌的地址仅保留在任务配置中。</p>
          {state.trackers.length ? <ul className="max-h-[140px] overflow-auto space-y-1 text-[11px] text-mist">{state.trackers.map((tracker, index) => <li key={`${index}:${tracker.tier}`} className="flex gap-2"><span className="min-w-0 flex-1 truncate">{tracker.tier} · {endpointLabel(tracker.url)}</span><span>{trackerStatus(tracker.status)} · 做种 {tracker.seeders < 0 ? '—' : tracker.seeders} / 下载 {tracker.leechers < 0 ? '—' : tracker.leechers}</span></li>)}</ul> : null}
        </div>
      </details>
      <div className="space-y-3 rounded-control border border-line p-3">
        <h4 className="text-[12px] font-medium text-paper">本任务分享策略</h4>
        <div className="grid grid-cols-2 gap-3">
          <label className="space-y-1 text-[12px] text-mist"><span>目标分享率</span><input aria-label="BT 目标分享率" className={input} type="number" min="0" max="1000000" step="0.1" value={draft.seedRatio} disabled={!editable} onChange={event => edit({ seedRatio: event.target.value })} /></label>
          <label className="space-y-1 text-[12px] text-mist"><span>做种分钟数</span><input aria-label="BT 做种分钟数" className={input} type="number" min="0" max={BT_MAX_SEED_MINUTES} step="1" placeholder="不设时间" value={draft.seedMinutes} disabled={!editable} onChange={event => edit({ seedMinutes: event.target.value })} /></label>
        </div>
        <p className="text-[11px] leading-relaxed text-mist">分享率 1 表示上传与下载等量，0 表示不按分享率停止。时间留空表示不设时间，0 表示下载后不做种；任一已设目标达到后结束分享。</p>
        <label className="block space-y-1 text-[12px] text-mist"><span>本任务上传上限（字节/秒，0 不限速）</span><input aria-label="BT 上传上限" className={input} type="number" min="0" max={BT_MAX_UPLOAD_LIMIT} step="1" value={draft.uploadLimit} disabled={!editable} onChange={event => edit({ uploadLimit: event.target.value })} /></label>
        <label className="flex gap-2 text-[12px] text-mist"><input type="checkbox" aria-label="BT 启用 PEX" checked={draft.peerExchange} disabled={!editable} onChange={event => edit({ peerExchange: event.target.checked })} />通过 PEX 交换 peer 信息（私有种子仍遵守引擎限制）</label>
      </div>
      {dirty ? <p role="status" className={`text-[11px] ${conflict ? 'text-clay' : 'text-mist'}`}>{conflict ? BT_ERROR_MESSAGES.conflict : '编辑尚未保存；刷新不会覆盖编辑内容。'}</p> : null}
      ...
      <details className="rounded-control border border-line p-3">
        <summary className="cursor-pointer text-[12px] font-medium text-paper">Peer · {state.peers.length} 个</summary>
        <div className="mt-3 space-y-3">
          <div className="max-h-[260px] overflow-auto"><table className="w-full text-left text-[11px] tabular-nums text-mist"><thead><tr><th className="py-1 font-normal">地址</th><th className="font-normal">下载 / 上传</th><th className="font-normal">状态</th></tr></thead><tbody>{state.peers.slice(page * 50, page * 50 + 50).map((peer, index) => <tr key={`${peer.ip}:${peer.port}:${index}`} className="border-t border-line"><td className="py-2 break-all">{peer.ip.includes(':') ? `[${peer.ip}]` : peer.ip}:{peer.port}<br />{(peer.progress * 100).toFixed(1)}%{peer.seeder ? ' · 做种' : ''}</td><td>{formatBytes(peer.downloadSpeed)}/s<br />{formatBytes(peer.uploadSpeed)}/s</td><td>{peerState(peer.state)}<br />{peer.encryption === 'plain' ? '未加密' : ['rc4', 'tls', 'encryptedHandshake'].includes(peer.encryption) ? '已加密' : '待确认'}</td></tr>)}</tbody></table>{!state.peers.length ? <p className="py-3 text-[12px] text-mist">当前没有 peer 连接。</p> : null}</div>
          {state.peers.length > 50 ? <div className="flex items-center justify-between"><button type="button" className={button} disabled={page === 0} onClick={() => setPeerPage(page - 1)}>上一页</button><span className="text-[11px] text-mist">{page + 1} / {Math.ceil(state.peers.length / 50)}</span><button type="button" className={button} disabled={(page + 1) * 50 >= state.peers.length} onClick={() => setPeerPage(page + 1)}>下一页</button></div> : null}
          <label className="block space-y-1 text-[12px] text-mist"><span>手动添加 peer · 每行 IP:端口或 [IPv6]:端口</span><textarea aria-label="手动 BT peer" className={`${input} min-h-[64px] font-mono`} value={peerText} disabled={!editable} spellCheck={false} onChange={event => setPeerText(event.target.value)} /></label>
    ...
    {global ? <div className="space-y-2 rounded-control border border-line p-3">
      <h4 className="text-[12px] font-medium text-paper">BT 会话加密 · 应用于全部 BT 任务</h4>
      <p className="text-[11px] text-mist">这项设置影响整个 BT 引擎，并在下次启动时保留。请先暂停全部 BT 任务。</p>
      ...
      {encryptionDirty ? <><label className="flex gap-2 text-[12px] text-mist"><input type="checkbox" checked={allConfirmed} disabled={busy} onChange={event => setAllConfirmed(event.target.checked)} />确认应用于全部 BT 任务</label>{globalConflict ? <p className="text-[11px] text-clay">{BT_ERROR_MESSAGES.conflict}</p> : null}<div className="flex flex-wrap gap-2"><button type="button" className={button} disabled={busy || !global.canConfigure || !allConfirmed || globalConflict} onClick={() => void run(async current => {
      ...
      {!global.canConfigure ? <p className="text-[11px] text-mist">还有 BT 任务未暂停，暂时不能更改。</p> : null}
    </div> : null}
    {notice ? <p role="status" className="text-[12px] text-mist">{notice}</p> : null}
