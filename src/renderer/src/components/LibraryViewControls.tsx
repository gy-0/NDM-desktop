          ...
          <Popover.Positioner sideOffset={8} align="end" className="z-[82] outline-none">
            <Popover.Popup className="library-filter-popup" data-library-filters>
              <div className="flex items-center justify-between gap-4">
                <Popover.Title className="text-lead font-semibold text-paper">{naming ? '保存常用视图' : '显示选项'}</Popover.Title>
              ...
              {naming ? (
                <form className="mt-3" onSubmit={event => {
                  ...
                  <p className="mb-4 text-body leading-relaxed text-mist">{viewCriteriaSummary(criteria)}</p>
                  <label className="grid gap-2 text-body text-fog">视图名称<input ref={nameInput} aria-label="视图名称" maxLength={80} value={name} onChange={event => { setName(event.target.value); setError(null) }} placeholder="例如：本周设计素材" className="saved-view-input" aria-invalid={Boolean(error)} aria-describedby={error ? 'save-view-error' : undefined} /></label>
                  {error ? <p id="save-view-error" role="status" className="mt-2 text-meta text-clay">{error}</p> : null}
                ...
                <>
                  ...
                  {criteria.time !== 'any' ? <p className="mt-2 text-meta leading-relaxed text-mist">按任务最近活动筛选，包含今天。</p> : null}
                  {criteria.query.trim() ? <p className="mt-3 truncate text-meta text-mist" title={criteria.query}>关键词：{criteria.query}</p> : null}
                  {activeViewName ? <p className="mt-3 text-meta text-mist">当前视图：{activeViewName}</p> : null}
