...
const buttonClass = 'rounded-control border border-line px-3 py-2 text-meta text-paper transition-colors hover:bg-line disabled:opacity-40'
...
export function DownloadImportPanel({ onCreated }: { onCreated?: (taskID: number) => void } = {}) {
  ...
  return <section aria-label="导入任务清单" className="mt-3 rounded-surface border border-line bg-panel/40 p-3.5">
    {!preview ? <>
      <p className="mb-3 text-meta leading-relaxed text-mist">选择 aria2 任务文件，或粘贴清单。每行一个任务，同一行的镜像地址用制表符分隔，任务选项在下一行缩进。</p>
      <textarea aria-label="aria2 任务清单" value={text} onChange={event => setText(event.target.value)} disabled={busy} rows={5} maxLength={1_048_576} spellCheck={false} placeholder={'https://example.com/file.zip\n  out=file.zip\n  pause=true'} className="w-full resize-y rounded-control border border-line bg-ink/40 p-3 font-mono text-meta text-paper outline-none focus:border-copper/50 disabled:opacity-40" />
      ...
      <p className="mt-2 text-caption text-mist">预览后会安全保存清单和创建记录；尚未预览的文本仍保留在当前输入框。</p>
    </> : <>
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="min-w-0"><p className="truncate text-body font-medium text-paper">{preview.sourceName}</p><p role="status" className="mt-1 text-meta text-mist">{preview.entries.length} 个任务 · 已创建 {accepted} 个</p></div>
      ...
      {recovered ? <p role="status" className="mt-2 text-meta text-mist">已恢复上次清单并核对创建回执；恢复过程不会提交新任务。</p> : null}
      {preview.entries.some(entry => entry.uris.length > 1) ? <p className="mt-2 text-meta leading-relaxed text-mist">镜像会在下载尚未写入数据时自动切换；已有续传数据时保留当前来源，重新下载后再尝试全部镜像。</p> : null}
      {preview.issues.length ? <ul role="alert" className="mt-3 space-y-1 text-meta text-clay">{preview.issues.map((issue, index) => <li key={index}>第 {issue.line} 行：{issue.message}</li>)}</ul> : null}
      <div className="my-3 flex items-center justify-between gap-2 text-meta text-mist">
          ...
          return <li key={entry.id} data-import-item={entry.id} className="p-3">
            <div className="flex items-start gap-2.5">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2"><p className="truncate text-body text-paper">{entry.fields.filename || entry.uris[0] || `第 ${entry.line} 行任务`}</p>{done ? <Check size={14} className="shrink-0 text-copper" aria-label="已创建" /> : activeID === entry.id ? <LoaderCircle size={14} className="shrink-0 animate-spin motion-reduce:animate-none" aria-label="正在确认" /> : null}</div>
                <p className="mt-1 text-caption text-mist">第 {entry.line} 行 · {entry.uris.length} 个地址 · {entry.options.length} 个选项{done ? ` · 任务 #${result.taskID}` : ''}</p>
                <details className="mt-2 text-meta text-mist"><summary className="cursor-pointer">查看地址、镜像和选项</summary><ul className="mt-2 space-y-1 break-all font-mono">{entry.uris.map((uri, index) => <li key={index}>{index ? `镜像 ${index}：` : '首选：'}{uri}</li>)}</ul>{entry.options.length ? <dl className="mt-2 space-y-1 break-all font-mono">{entry.options.map((option, index) => <div key={index}><dt className="inline">{option.name}=</dt><dd className="inline">{option.value}</dd></div>)}</dl> : null}</details>
                {entry.issues.map((issue, index) => <p key={index} className="mt-1 text-meta text-clay">第 {issue.line} 行：{issue.message}</p>)}
                {result?.error ? <p role="status" className="mt-2 text-meta text-clay">{result.error}</p> : null}
              </div>
            </div>
          </li>
        })}
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
        <label className="flex items-center gap-2 text-meta text-mist"><input type="checkbox" checked={autoStart} onChange={event => setAutoStart(event.target.checked)} disabled={busy || attempted} />创建后开始下载（以每项 pause 选项为准）</label>
      ...
      {uncertain ? <div className="mt-2 space-y-2"><p className="text-meta text-mist">部分结果尚未确认。原清单已保存，恢复时会核对已记录的创建操作。已创建的任务会保留。</p><button type="button" onClick={() => void resume()} disabled={busy} className={buttonClass}>仅核对回执</button></div> : null}
    </>}
    {error ? <p role="alert" className="mt-3 text-meta text-clay">{error}</p> : null}
  </section>
}
