  ...
  return (
    <Dialog.Root open={open} onOpenChange={(next) => { if (!next) onClose() }}>
      <Dialog.Portal>
        ...
        <Dialog.Viewport className="workspace-dialog-viewport" data-anchor="top">
          <Dialog.Popup
            data-command-palette
            initialFocus={input}
            finalFocus={() => executed.current ? false : true}
            aria-describedby={selectionLabel ? contextId : undefined}
            className="workspace-dialog-popup flex w-[min(620px,100%)] flex-col overflow-hidden rounded-surface border border-line-strong bg-raised shadow-dialog"
          ...
          >
            <div className="shrink-0 border-b border-line px-5 pt-4 pb-3">
              <div className="mb-3 flex items-center justify-between gap-3">
                <Dialog.Title className="text-heading font-semibold tracking-[-0.01em] text-paper">快速操作</Dialog.Title>
                <Dialog.Close aria-label="关闭快速操作" className="rounded-control p-1 text-mist transition-colors hover:bg-line hover:text-paper">
              ...
              <div className="flex items-center gap-3">
                ...
                <input
                  ...
                  className="min-w-0 flex-1 bg-transparent py-1 text-heading leading-6 text-paper outline-none placeholder:text-mist"
                />
                {query ? <button type="button" aria-label="清空操作搜索" onClick={() => { followActive.current = true; setQuery(''); setActiveId(null); input.current?.focus() }} className="rounded-control p-1 text-mist hover:bg-line hover:text-paper"><X size={14} aria-hidden="true" /></button> : null}
              </div>
            </div>

            {selectionLabel ? (
              <p id={contextId} data-command-context className="flex min-w-0 shrink-0 items-center gap-2 border-b border-line/60 bg-panel/40 px-5 py-2.5 text-body leading-5 text-fog">
            ...
            <div id={listId} role="listbox" aria-label="可用操作" className="scroll-quiet min-h-32 overflow-y-auto">
              ...
              {groups.map((group) => group.items.length > 0 ? (
                <div key={group.scope} role="group" aria-labelledby={`${id}-${group.scope}`} className="mb-1 last:mb-0">
                  <div id={`${id}-${group.scope}`} className="flex items-center gap-2 px-3 pt-2.5 pb-1.5 text-meta font-medium text-mist">
                  ...
                  {group.items.map((item) => {
                    ...
                    return <button
                      ...
                      onClick={() => select(item)}
                      className="command-option flex w-full items-center gap-4 rounded-control border border-transparent px-3 py-2.5 text-left disabled:cursor-default disabled:opacity-40"
                    >
                      <span className="min-w-0 flex-1">
                        <span className="block truncate text-heading font-medium leading-6 text-paper">{item.label}</span>
                        {item.detail ? <span className="mt-0.5 block truncate text-label leading-5 text-mist">{item.detail}</span> : null}
                      </span>
                      {item.shortcut ? <kbd className="shrink-0 rounded-control border border-line-strong bg-panel/50 px-1.5 py-0.5 font-sans text-meta leading-5 text-fog">{item.shortcut}</kbd> : <CornerDownLeft size={15} className={`shrink-0 text-fog ${isActive ? '' : 'invisible'}`} aria-hidden="true" />}
                    </button>
                  })}
                </div>
              ) : null)}
              {ordered.length === 0 ? <div role="status" className="px-4 py-9 text-center">
                <p className="text-lead font-medium text-paper">没有找到这个操作</p>
                <p className="mt-2 text-body leading-5 text-mist">换个关键词试试，例如“设置”。</p>
              </div> : null}
                </div>
              </AnimatedHeight>
            </div>
            <div className="flex shrink-0 items-center justify-between gap-4 border-t border-line px-5 py-3 text-meta leading-4 text-mist">
