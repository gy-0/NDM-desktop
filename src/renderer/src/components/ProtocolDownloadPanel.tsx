...
const control = 'inline-flex items-center justify-center gap-1.5 rounded-control border border-line px-3 py-2 text-meta text-paper hover:bg-raised disabled:opacity-40'
const input = 'w-full rounded-control border border-line bg-raised px-2.5 py-2 text-meta text-paper outline-none focus:border-copper/50 disabled:opacity-40'
...
export function ProtocolDownloadPanel({ onCreated, initialURL = '' }: { onCreated?: (taskID: number) => void; initialURL?: string } = {}) {
  ...
  return <section aria-label="协议下载" className="space-y-3 rounded-surface border border-line bg-panel/40 p-3.5">
    <div><h3 className="text-body font-medium text-paper">协议下载</h3><p className="mt-1 text-meta leading-relaxed text-mist">添加磁力链接、种子文件、ED2K 或 SFTP 下载。</p></div>
    ...
    {!creation ? <>
      {!capabilitiesReady && !busy ? <p role="status" className="text-meta text-clay">当前设备的引擎尚不支持{isBT ? '安全的 BT 文件选择流程' : protocol.toUpperCase()}，此入口暂不可创建任务。</p> : null}
      ...
      {protocol === 'sftp' ? <div className="space-y-2 rounded-control border border-line p-3">
        ...
        <label className="block space-y-1 text-meta text-mist"><span>服务器公钥 SHA-256 指纹（必填）</span><input aria-label="SFTP 主机指纹" value={hostPin} onChange={event => setHostPin(event.target.value)} disabled={locked || !capabilitiesReady} maxLength={128} spellCheck={false} autoComplete="off" placeholder="SHA256:…" className={`${input} font-mono`} /></label>
        {hostPin && !normalizeSFTPHostPin(hostPin) ? <p className="text-caption text-clay">指纹必须包含 32 字节 SHA-256 摘要的 Base64 值。</p> : null}
        <p className="text-caption leading-relaxed text-mist">请从服务器管理员或可信渠道核实指纹。指纹不符时停止连接；用户名和密码仅保存在当前应用内存中，任务确认创建后即清除表单副本。</p>
      </div> : null}
      <div className="flex flex-wrap items-center gap-2"><button type="button" className={control} disabled={locked || !capabilitiesReady} onClick={() => void run(async () => { const path = await window.ndm?.selectFolder(folderPath || undefined); if (path && mounted.current) { setFolderPath(path); saveDirectoryShortcut(path) } })}><FolderOpen size={14} />选择目标目录</button>{folderPath ? <><span className="break-all text-meta text-fog">{folderPath}</span><button type="button" className={control} disabled={locked} onClick={() => setFolderPath('')}>使用默认目录</button></> : <span className="text-meta text-mist">使用默认目录与目录规则</span>}</div>
      <DirectoryShortcuts currentDirectory={folderPath} disabled={locked} onChoose={setFolderPath} />
      {!isBT ? <label className="flex items-center gap-2 text-meta text-mist"><input type="checkbox" checked={autoStart} onChange={event => setAutoStart(event.target.checked)} disabled={locked || !capabilitiesReady} />创建后开始下载</label> : <p className="text-meta text-mist">先读取元数据，再到任务详情选择文件；确认选择前不下载文件内容。</p>}
      ...
      <p className="text-meta text-paper">{creation.label}</p>
      {creation.phase === 'accepted' ? <><p role="status" className="text-meta text-fog">已确认创建任务 #{creation.taskID}。BT 任务可在详情中选择文件。</p><button type="button" className={control} disabled={busy} onClick={() => { pendingCreation = null; setCreation(null); setError('') }}>添加另一任务</button></>
        : creation.phase === 'rejected' ? <><p role="status" className="text-meta text-clay">{creation.message}</p><button type="button" className={control} disabled={busy} onClick={() => { pendingCreation = null; setCreation(null); setError('') }}>修改参数</button></>
        : <><p role="status" className="text-meta text-mist">{creation.message ?? '正在核对并提交任务…'}</p><div className="flex flex-wrap gap-2"><button type="button" className={control} disabled={busy} onClick={() => void run(async () => { const state = await pendingCreation!.reconcile(false); if (mounted.current) acceptCreation(state) })}>仅核对回执</button><button type="button" className={control} disabled={busy} onClick={() => void run(async () => { const state = await pendingCreation!.reconcile(true); if (mounted.current) acceptCreation(state) })}>核对并重试原请求</button></div><p className="text-caption text-mist">结果未确认时保留同一请求标识。关闭并重开此面板会先核对回执。</p></>}
    </div>}
