        ...
        <Dialog.Viewport className="workspace-dialog-viewport">
          <Dialog.Popup className="workspace-dialog-popup saved-views-dialog">
            <header className="flex items-start justify-between gap-4 p-5 pb-4">
              <div><Dialog.Title className="text-headline font-semibold tracking-[-0.02em] text-paper">常用视图</Dialog.Title><Dialog.Description className="mt-1.5 text-body text-mist">保存常用的筛选和排序，下次直接打开。</Dialog.Description></div>
                ...
                <li key={view.id} data-saved-view={view.id} className="saved-view-row" data-current={view.id === activeId || undefined}>
                  {editing === view.id ? <form className="saved-view-rename" onSubmit={event => {
                    ...
                    <button type="button" className="saved-view-open" onClick={() => { onApply(view); onClose() }} aria-label={`打开视图：${view.name}`}>
                      ...
                      <span className="min-w-0 flex-1"><span className="flex min-w-0 items-center gap-2"><span className="truncate text-lead font-medium text-paper">{view.name}</span>{view.id === activeId ? <Check size={14} className="shrink-0 text-mist" aria-label="当前视图" /> : null}</span><span className="mt-1 line-clamp-2 break-words text-meta leading-relaxed text-mist" title={`${viewCriteriaSummary(view.criteria)} · ${viewSortSummary(view.sort)}`}>{viewCriteriaSummary(view.criteria)} · {viewSortSummary(view.sort)}</span></span>
                    </button>
            ...
            </ul> : <div className="grid justify-items-center px-8 py-10 text-center"><span aria-hidden className="saved-view-mark mb-4"><Bookmark size={22} strokeWidth={1.5} /></span><h3 className="text-heading font-medium text-paper">把常找的下载放在手边</h3><p className="mt-2 max-w-[300px] text-body leading-relaxed text-mist">在列表的“筛选”中选好条件，再保存为常用视图。</p></div>}
            {localError ? <p role="status" className="px-5 py-3 text-body text-clay">{localError}</p> : null}
          </Dialog.Popup>
        </Dialog.Viewport>
      </Dialog.Portal>
    </Dialog.Root>
