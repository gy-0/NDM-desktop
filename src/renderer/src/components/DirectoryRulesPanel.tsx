...
const buttonClass = 'inline-flex items-center justify-center gap-1.5 rounded-control border border-line-strong px-2.5 py-1.5 text-[12px] text-fog hover:bg-line disabled:opacity-40'
const inputClass = 'w-full rounded-control border border-line bg-ink/40 px-2.5 py-2 text-[12px] text-paper outline-none focus:border-copper/50 disabled:opacity-40'
...
export function DirectoryRulesPanel({ onApplied }: { onApplied?: () => void | Promise<void> } = {}) {
  ...
  return <section aria-label="下载目录规则" className="space-y-3">
    <div className="flex items-start justify-between gap-3">
      <div><h3 className="text-[13px] font-medium text-paper">下载目录规则</h3><p className="mt-1 text-[12px] leading-relaxed text-mist">按顺序使用第一条匹配规则。手动选定的目录优先；未匹配时沿用默认目录和分类设置。</p></div>
      <label className="flex shrink-0 items-center gap-2 text-[12px] text-fog"><input type="checkbox" aria-label="启用下载目录规则" checked={enabled} disabled={busy || revision === null} onChange={event => { setEnabled(event.target.checked); edited() }} />启用</label>
    </div>
    <p className="text-[11px] leading-relaxed text-mist">每组条件满足任意一项，填写多组时须同时满足。主机不区分大小写；URL 路径使用地址中的编码形式，区分大小写，不含查询参数。* 匹配任意字符串，? 匹配一个字符。扩展名只匹配最后一段，无扩展名不匹配扩展名条件。</p>
    {!rules.length ? <p className="rounded-control border border-dashed border-line-strong p-3 text-[12px] text-mist">尚无规则。添加规则并保存后，仅影响新建任务的目标目录。</p> : null}
    <ol className="space-y-3">{rules.map((rule, index) => <li key={rule.id} className="space-y-2 rounded-control border border-line-strong p-3">
      <div className="flex items-center gap-2">
        <span className="text-[11px] text-mist">{index + 1}</span>
      ...
      <div className="grid gap-2 sm:grid-cols-3">
        <label className="space-y-1 text-[11px] text-mist"><span>主机（逗号或换行分隔）</span><textarea aria-label={`规则 ${index + 1} 主机`} rows={2} maxLength={2048} value={rule.hostsText} disabled={busy} onChange={event => change(rule.id, { hostsText: event.target.value })} placeholder="*.example.com" className={`${inputClass} resize-y font-mono`} /></label>
        <label className="space-y-1 text-[11px] text-mist"><span>URL 路径（每行一项）</span><textarea aria-label={`规则 ${index + 1} 路径`} rows={2} maxLength={2048} value={rule.pathsText} disabled={busy} onChange={event => change(rule.id, { pathsText: event.target.value })} placeholder="/releases/*" className={`${inputClass} resize-y font-mono`} /></label>
        <label className="space-y-1 text-[11px] text-mist"><span>扩展名（逗号或换行分隔）</span><textarea aria-label={`规则 ${index + 1} 扩展名`} rows={2} maxLength={2048} value={rule.extensionsText} disabled={busy} onChange={event => change(rule.id, { extensionsText: event.target.value })} placeholder="zip, dmg" className={`${inputClass} resize-y font-mono`} /></label>
      </div>
      <div className="flex items-center gap-2"><p className="min-w-0 flex-1 break-all text-[12px] text-fog">{rule.directory}</p><button type="button" disabled={busy} className={buttonClass} onClick={() => void run(async () => { const directory = await chooseDirectory(); if (directory && mounted.current) change(rule.id, { directory }) })}><FolderOpen size={13} />更改目录</button></div>
      {!isDirectoryForPlatform(rule.directory, window.ndm?.platform === 'win32' ? 'win32' : 'posix') ? <p className="text-[11px] text-clay">此目录不适用于当前系统，匹配时会跳过这条规则。</p> : null}
    </li>)}</ol>
    ...
    <div className="space-y-2 rounded-control border border-line p-3">
      <p className="text-[12px] font-medium text-paper">预览当前编辑的规则</p>
      ...
      <div className="flex flex-wrap items-center gap-2"><button type="button" disabled={busy} className={buttonClass} onClick={() => void run(async () => { const directory = await chooseDirectory(); if (directory && mounted.current) { setExplicitDirectory(directory); setResolution(null) } })}>模拟手动选择目录</button>{explicitDirectory ? <><span className="break-all text-[11px] text-mist">{explicitDirectory}</span><button type="button" disabled={busy} className={buttonClass} onClick={() => { setExplicitDirectory(''); setResolution(null) }}>清除</button></> : null}</div>
      ...
      {resolution ? <div role="status" className="space-y-1 text-[12px] text-fog"><p>{resolution.source === 'explicit' ? '使用手动目录' : resolution.source === 'rule' ? `匹配规则：${resolution.ruleName}` : '使用现有默认目录 / 分类设置'}</p><p className="break-all text-paper">{resolution.directory}</p>{resolution.ignoredRuleIDs?.length ? <p className="text-[11px] text-mist">已跳过 {resolution.ignoredRuleIDs.length} 条目录不适用于当前系统的规则。</p> : null}</div> : null}
      <p className="text-[11px] text-mist">预览不会创建目录或下载任务，也不会保存样例地址。</p>
    </div>
