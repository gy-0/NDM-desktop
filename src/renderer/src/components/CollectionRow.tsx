...
export function CollectionRow({
  ...
  return (
    <div
      ...
      className="group relative rounded-control border border-line/55 bg-raised/22 transition-[background-color,border-color,box-shadow] duration-150 hover:border-line-strong/60 hover:bg-raised/44 hover:shadow-row"
    >
      <button
        type="button"
      ...
      >
        <span data-collection-heading className="flex min-w-0 items-center gap-3 px-3 pe-5">
          ...
          <span className="min-w-0">
            <span className="block truncate text-lead font-medium text-paper/96" title={title}>{title}</span>
            <span
              id={`collection-action-status-${collectionID}`}
              role={groupActionError ? 'status' : undefined}
              aria-live="polite"
              className={`mt-1.5 flex items-center gap-1.5 text-meta ${groupActionError ? 'text-clay' : 'text-fog'}`}
            >
        ...
        <span className="font-sans text-meta tabular-nums text-mist">{completed === count ? '完成' : `${completed}/${count}`}</span>
        ...
        <span className="whitespace-nowrap pe-4 text-right text-meta tabular-nums text-mist" title={latestActivityAt ? new Date(latestActivityAt).toLocaleString('zh-CN') : undefined}>
        ...
        <span className="task-row-progress flex items-center gap-2.5 !pe-12">
          {completed < count && fraction > 0 ? (
            <>
              <span className="w-9 text-end font-sans text-meta tabular-nums text-mist">{Math.round(fraction * 100)}%</span>
