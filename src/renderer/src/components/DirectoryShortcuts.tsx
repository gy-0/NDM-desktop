...
export function DirectoryShortcuts({ currentDirectory, disabled, onChoose, onUseDefault }: {
  ...
  const row = (path: string) => <li key={path} className="flex min-w-0 items-center gap-1">
    <button type="button" disabled={disabled} title={path} className="min-w-0 flex-1 truncate rounded px-2 py-1 text-left text-[12px] text-fog hover:bg-line disabled:opacity-40" onClick={() => { saveDirectoryShortcut(path); onChoose(path) }}>{path}</button>
  ...
  return <div className="flex flex-wrap items-start gap-3 text-[12px]">
    ...
    {favorites.length || recent.length ? <details className="min-w-0 flex-1 rounded-control border border-line px-2 py-1"><summary className="cursor-pointer text-mist">收藏与最近目录</summary>
      <div className="mt-2 max-h-48 overflow-auto">{favorites.length ? <><p className="px-2 text-[11px] text-mist">收藏</p><ul>{favorites.map(row)}</ul></> : null}{recent.length ? <><p className="mt-2 px-2 text-[11px] text-mist">最近使用</p><ul>{recent.map(row)}</ul></> : null}</div>
    </details> : null}
  </div>
}
