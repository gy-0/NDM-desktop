...
export function AppUpdatePanel() {
  ...
  return <div className="space-y-2 pt-3" aria-label="版本更新">
    ...
    {message ? <p role={reply?.status === 'error' ? 'alert' : 'status'} className={`text-body leading-relaxed ${reply?.status === 'error' ? 'text-clay' : 'text-fog'}`}>{message}</p> : <p className="text-meta text-mist">点击后检查 GitHub 上的公开正式版。</p>}
    {reply && reply.status !== 'error' ? <p className="text-meta text-mist">检查时间：{new Date(reply.checkedAt).toLocaleTimeString()}</p> : null}
    {openError ? <p role="alert" className="text-body text-clay">未能打开发行说明，请在浏览器访问 github.com/gy-0/NDM-desktop/releases。</p> : null}
  </div>
}
