...
const control = 'inline-flex items-center justify-center gap-1.5 rounded-control border border-line px-2.5 py-2 text-meta text-paper hover:bg-raised disabled:opacity-40'
...
export function AuxiliaryTransferPanel({ taskID, onChanged }: { taskID: number; onChanged?: () => void | Promise<void> }) {
  ...
  return <section aria-label="辅助协议任务" className="space-y-3 border-t border-line pt-4">
    <div className="flex items-center justify-between gap-2"><h3 className="text-body font-medium text-paper">协议任务详情</h3><button type="button" className={control} disabled={busy || loading} onClick={() => void run(async () => { await refresh(generation.current) })}><RefreshCw size={13} />刷新状态</button></div>
    {loading ? <p role="status" className="text-meta text-mist">正在读取协议状态…</p> : null}
    {snapshot ? <>
      <div role="status" className="flex flex-wrap items-center justify-between gap-2 text-meta"><span className={snapshot.phase === 'error' ? 'text-clay' : 'text-paper'}>{snapshot.kind === 'ed2k' && snapshot.phase === 'seeding' ? '正在共享' : AUXILIARY_PHASE_LABELS[snapshot.phase]}</span><span className="tabular-nums text-mist">{formatBytes(snapshot.completedBytes)} / {formatBytes(snapshot.totalBytes)}</span></div>
      {snapshot.phase === 'metadata' ? <p className="text-meta text-mist">正在获取文件清单，元数据完成后会暂停等待文件选择。</p> : null}
      {snapshot.phase === 'awaitingSelection' ? <p className="text-meta text-mist">至少选择一个文件并确认，才会开始下载文件内容。</p> : null}
      {snapshot.phase === 'error' || snapshot.errorCode ? <p className="text-meta text-clay">{auxiliaryErrorMessage({ code: snapshot.errorCode }, `下载未能完成${snapshot.errorCode ? `（错误码 ${snapshot.errorCode}）` : ''}。请检查任务来源和连接设置。`)}</p> : null}
      <div className="grid grid-cols-2 gap-2 text-meta text-mist"><span>下载 {formatBytes(snapshot.downloadSpeed)}/s</span><span>上传 {formatBytes(snapshot.uploadSpeed)}/s</span></div>
      {snapshot.kind === 'sftp' && ['paused', 'error'].includes(snapshot.phase) ? <div className="space-y-2 rounded-control border border-line p-3">
        <p className="text-meta text-paper">继续 SFTP 任务</p>
        <p className="text-caption leading-relaxed text-mist">重启应用后请重新输入账号密码。凭据仅保存在本次运行的内存中；继续使用创建任务时已核实的服务器指纹。</p>
        <input aria-label="继续 SFTP 的用户名" value={username} onChange={event => setUsername(event.target.value)} maxLength={256} disabled={busy} autoComplete="off" placeholder="用户名" className="w-full rounded-control border border-line bg-raised px-2.5 py-2 text-meta text-paper" />
        <input aria-label="继续 SFTP 的密码" type="password" value={password} onChange={event => setPassword(event.target.value)} maxLength={4096} disabled={busy} autoComplete="new-password" placeholder="密码" className="w-full rounded-control border border-line bg-raised px-2.5 py-2 text-meta text-paper" />
      ...
      {snapshot.kind === 'bittorrent' || snapshot.kind === 'ed2k' ? <>
        <div className="grid grid-cols-2 gap-2 text-meta text-mist"><span>累计上传 {snapshot.uploadedBytes === undefined ? '暂不可用' : formatBytes(snapshot.uploadedBytes)}</span><span>分享率 {snapshot.ratio === undefined ? '暂不可用' : snapshot.ratio.toFixed(2)}</span></div>
        {snapshot.phase === 'seeding' ? <div className="space-y-2"><p className="text-meta text-mist">文件内容已下载，仍在向其他节点上传。停止做种后再核对任务交付状态。</p><button type="button" disabled={busy || capabilities?.stopSeeding !== true} className={control} onClick={() => void run(async () => {
          const current = generation.current, captured = currentSnapshot.current!
          let reply: unknown
          ...
          await refresh(current, true); await changed()
        })}><Square size={13} />{snapshot.kind === 'ed2k' ? '停止共享' : '停止做种'}</button>{capabilities?.stopSeeding !== true ? <p className="text-caption text-mist">当前引擎未提供停止共享操作。</p> : null}</div> : null}
        {snapshot.kind === 'bittorrent' && snapshot.files.length ? <>
          ...
          {capabilities?.fileSelection !== true ? <p className="text-meta text-mist">当前引擎未提供文件选择能力。</p> : null}
          <input aria-label="查找种子文件" value={search} onChange={event => setSearch(event.target.value)} placeholder="按文件路径查找" className="w-full rounded-control border border-line bg-raised px-2.5 py-2 text-meta text-paper outline-none focus:border-copper/50" />
          <div className="flex items-center justify-between gap-2 text-meta text-mist"><label className="flex items-center gap-2"><input type="checkbox" aria-label="选择全部显示的文件" disabled={!canSelect || busy || !files.length} checked={files.length > 0 && files.every(file => selected.has(file.index))} onChange={event => {
          ...
          <div ref={scrollElement} className="h-[280px] overflow-auto rounded-control border border-line">
            <ul style={{ height: virtualizer.getTotalSize(), position: 'relative' }}>{virtualizer.getVirtualItems().map(virtual => {
              ...
              return <li key={file.index} data-index={virtual.index} ref={virtualizer.measureElement} style={{ position: 'absolute', top: 0, left: 0, width: '100%', transform: `translateY(${virtual.start}px)` }} className="border-b border-line p-2.5">
                ...
                }} /><span className="min-w-0"><span className="block break-all text-meta text-paper">{file.relativePath}</span><span className="mt-1 block text-caption tabular-nums text-mist">{formatBytes(file.completedLength)} / {formatBytes(file.length)}</span></span></label>
              </li>
            })}</ul>
          </div>
          {canSelect ? <div className="space-y-2"><label className="flex items-center gap-2 text-meta text-mist"><input type="checkbox" checked={autoStart} onChange={event => setAutoStart(event.target.checked)} disabled={busy} />确认后开始下载</label>{selectionDirty ? <p className="text-caption text-mist">文件选择尚未提交。</p> : null}<button type="button" disabled={busy || selected.size === 0} className={`${control} border-copper/30 bg-copper/10`} onClick={() => void run(async () => {
            const current = generation.current
