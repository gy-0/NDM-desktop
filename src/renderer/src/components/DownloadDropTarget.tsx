  ...
  return <AnimatePresence>
    {active ? <motion.div key="download-drop-target" data-download-drop-target
      ...
      <div ref={targetRef} data-drop-card data-drop-hot={hot || undefined}
        className="download-drop-card relative flex w-[min(460px,calc(100%-48px))] items-start gap-4 rounded-xl border border-line-strong bg-raised px-6 py-5 shadow-dialog">
        ...
        <div className="min-w-0">
          <div className="text-[18px] font-semibold leading-tight text-paper">释放以检查下载</div>
          <p className="mt-1.5 text-[12px] leading-relaxed text-mist">支持网页、文件直链、媒体链接和磁力链；确认后再开始</p>
        </div>
      </div>
    </motion.div> : null}
  </AnimatePresence>
