        ...
        {links.map((item, index) => {
          ...
          return (
            <li key={item.url} data-batch-link className="flex min-w-0 items-center gap-3 px-3.5 py-2.5">
              <span aria-hidden className="grid size-8 shrink-0 place-items-center rounded-control border border-line bg-panel/60 text-fog"><FileDown size={16} strokeWidth={1.6} /></span>
              <div className="min-w-0 flex-1">
                <p className="truncate text-body font-medium text-paper" title={item.url}>{identity.title}</p>
                <div className="mt-0.5 flex min-w-0 items-center gap-2 text-meta">
